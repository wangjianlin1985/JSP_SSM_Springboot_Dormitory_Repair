...
public class StudentController extends BaseController {
	...
	public void list(String studentNo,String speicalName,String gradeInfo,String name,String birthDate,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (studentNo == null) studentNo = "";
		if (speicalName == null) speicalName = "";
		if (gradeInfo == null) gradeInfo = "";
		if (name == null) name = "";
		if (birthDate == null) birthDate = "";
		if (telephone == null) telephone = "";
		if(rows != 0)studentService.setRows(rows);
		List<Student> studentList = studentService.queryStudent(studentNo, speicalName, gradeInfo, name, birthDate, telephone, page);
	    /*计算总的页数和总的记录数*/
	    studentService.queryTotalPageAndRecordNumber(studentNo, speicalName, gradeInfo, name, birthDate, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String studentNo,String speicalName,String gradeInfo,String name,String birthDate,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (birthDate == null) birthDate = "";
		if (telephone == null) telephone = "";
		List<Student> studentList = studentService.queryStudent(studentNo, speicalName, gradeInfo, name, birthDate, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    studentService.queryTotalPageAndRecordNumber(studentNo, speicalName, gradeInfo, name, birthDate, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = studentService.getTotalPage();
	    ...
	    request.setAttribute("speicalName", speicalName);
	...
	public void OutToExcel(String studentNo,String speicalName,String gradeInfo,String name,String birthDate,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(studentNo == null) studentNo = "";
        if(speicalName == null) speicalName = "";
        if(gradeInfo == null) gradeInfo = "";
        if(name == null) name = "";
        if(birthDate == null) birthDate = "";
        if(telephone == null) telephone = "";
        List<Student> studentList = studentService.queryStudent(studentNo,speicalName,gradeInfo,name,birthDate,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Student信息记录"; 
        String[] headers = { "学号","所在专业","年级","姓名","性别","出生日期","学生照片","联系电话","邮箱","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<studentList.size();i++) {
        	Student student = studentList.get(i); 
        	dataset.add(new String[]{student.getStudentNo(),student.getSpeicalName(),student.getGradeInfo(),student.getName(),student.getGender(),student.getBirthDate(),student.getStudentPhoto(),student.getTelephone(),student.getEmail(),student.getRegTime()});
        }
        /*
