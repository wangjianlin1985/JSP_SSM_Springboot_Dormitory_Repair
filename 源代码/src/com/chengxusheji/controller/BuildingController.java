...
public class BuildingController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询宿舍楼信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String buildingName,String buildingType,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (buildingName == null) buildingName = "";
		if (buildingType == null) buildingType = "";
		if(rows != 0)buildingService.setRows(rows);
		List<Building> buildingList = buildingService.queryBuilding(buildingName, buildingType, page);
	    /*计算总的页数和总的记录数*/
	    buildingService.queryTotalPageAndRecordNumber(buildingName, buildingType);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String buildingName,String buildingType,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (buildingName == null) buildingName = "";
		if (buildingType == null) buildingType = "";
		List<Building> buildingList = buildingService.queryBuilding(buildingName, buildingType, currentPage);
	    /*计算总的页数和总的记录数*/
	    buildingService.queryTotalPageAndRecordNumber(buildingName, buildingType);
	    /*获取到总的页码数目*/
	    int totalPage = buildingService.getTotalPage();
	...
	public String delete(@PathVariable Integer buildingId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  buildingService.deleteBuilding(buildingId);
	            request.setAttribute("message", "宿舍楼删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "宿舍楼删除失败!");
				return "error";
	...
	public void OutToExcel(String buildingName,String buildingType, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(buildingName == null) buildingName = "";
        if(buildingType == null) buildingType = "";
        List<Building> buildingList = buildingService.queryBuilding(buildingName,buildingType);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Building信息记录"; 
        String[] headers = { "宿舍楼id","宿舍楼名称","宿舍楼类型"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<buildingList.size();i++) {
        	Building building = buildingList.get(i); 
        	dataset.add(new String[]{building.getBuildingId() + "",building.getBuildingName(),building.getBuildingType()});
        }
        /*
